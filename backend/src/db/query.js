...
export const updateArticleFromDb = async ({
  title,
  content,
  excerpt,
  id,
  ...
  const result = await db.query(
    "UPDATE articles SET title = $1, content = $2,excerpt=$3, imageurl=$5 WHERE id = $4 RETURNING *",
    [title, content, excerpt, id, imageUrl],
  );
  return result.rows[0];
};
